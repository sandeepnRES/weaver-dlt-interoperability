...
const interopHelper = async (
  ...
  try {
    ...
    logger.info(`ViewB64: ${Buffer.from(interopFlowResponse.views[0].serializeBinary()).toString('base64')}`)
    ...
    if (remoteValue.contents) {
        logger.info(`ViewB64Contents: ${Buffer.from(remoteValue.contents).toString('base64')}`)
    }
    spinner.succeed(
